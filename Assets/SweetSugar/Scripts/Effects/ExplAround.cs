    ...
    public class ExplAround : MonoBehaviour
    {
        ...
        private void Play(Item[] array, float delay, float force)
        {
            foreach (Item item1 in array)
            {
                ...
                child.gameObject.transform.DOMove(v, duration / 2).SetDelay(delay).SetEase(curve).OnComplete(
                    () => { OnFinished(1); });
            }

            DOVirtual.DelayedCall(5, Destr);
        }
...
//            item.SetActive(false);
        ...
        private void Start()
        {
            ...
            DOVirtual.DelayedCall(startParticle, () => { particle.SetActive(true); });
