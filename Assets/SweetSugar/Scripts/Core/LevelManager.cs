                ...
                switch (value)
                {
                    ...
                    case GameState.Win: //shows MenuComplete
                        ...
                        if (winRewardAmount > 0)
                            InitScript.Instance
                                .ShowGemsReward(winRewardAmount); // InitScript.Instance.ShowGemsReward(10);
                        OnWin();
                        break;
                }
            }
...
//            GameObject.Find("CanvasMap/SafeArea/Gems").gameObject.SetActive(false);
        ...
        private void PrepareGame()
        {
            ...
            {
                InGameBoosts.Where(i => i.type == BoostType.ExtraMoves).ToList()
                    .ForEach(i => i.gameObject.SetActive(true));
                InGameBoosts.Where(i => i.type == BoostType.ExtraTime).ToList()
        ...
        public void CheckWinLose()
        {
            ...
            {
                Debug.Log((win ? "win " : "lose ") + " score " + Score + " stars " + stars + " moves/time rest " +
                          THIS.levelData.limit);
                RestartLevel();
            }
        }
        ...
        private IEnumerator PreWinLoop(int limit)
        {
            ...
            do
            {
                while (field.GetAllExtaItems().Count > 0 && gameStatus != GameState.Win)
                {
                    ...
                    else if (item.currentType == ItemsTypes.MULTICOLOR)
                        item.Check(item, field.GetRandomItems(1).First());
                    dragBlocked = true;
                    if (skipWin) yield break;
        ...
        void MouseDown(Vector2 pos)
        {
            ...
            if (hit == null && AdditionalSettings.SelectableSolidBlock)
            {
                ...
                if (hit1 != null)
                {
                    ...
                    if (fakeItemSquare.type == SquareTypes.SolidBlock ||
                        fakeItemSquare.type == SquareTypes.SpiralBlock ||
                        fakeItemSquare.type == SquareTypes.ThrivingBlock)
                    {
            ...
            if (hit != null)
            {
                    ...
                    (gameStatus == GameState.Playing || gameStatus == GameState.Tutorial))
                {
                    ...
                    if (THIS.ActivatedBoost.type == BoostType.ExplodeArea &&
                        lastTouchedItem.currentType != ItemsTypes.MULTICOLOR &&
                        lastTouchedItem.currentType != ItemsTypes.INGREDIENT)
                    {
                        SoundBase.Instance.PlayOneShot(SoundBase.Instance.boostBomb);
                        THIS.DragBlocked = true;
                        var obj = Instantiate(Resources.Load("Boosts/area_explosion"),
                            lastTouchedItem.transform.position,
                             ...
                             lastTouchedItem.currentType != ItemsTypes.INGREDIENT)
                    {
                        ...
                        THIS.DragBlocked = true;
                        var obj = Instantiate(Resources.Load("Boosts/simple_explosion"),
                            lastTouchedItem.transform.position,
                            lastTouchedItem.transform.rotation) as GameObject;
                        obj.GetComponent<BoostAnimation>().square = lastTouchedItem.square;
                        obj.GetComponent<SpriteRenderer>().sortingOrder = 4;
...
//            DestroyItems(true);
        ...
        private IEnumerator RegenMatches(bool onlyFalling = false)
        {
            ...
            field.GetItems().ForEach(i =>
            {
                ...
                transform.DOScale(transformLocalScale, .3f);
                i.Hide(false);
            });
        }
...
//        Debug.Log("<-next turn->");
            if (gameStatus == GameState.Playing)
            {
                ...
                else
                {
                    yield return new WaitWhile(() =>
                        LevelManager.THIS.animateItems.Any() || LevelManager.THIS.checkTarget);
                    OnTurnEnd?.Invoke();
                    THIS.CheckWinLose();
                }
            }
        ...
        public List<Square> GetRowSquare(int row)
        {
            ...
            for (var col = 0; col < levelData.maxCols; col++)
            {
                ...
                if (square.type == SquareTypes.UndestrBlock &&
                    LevelManager.THIS.AdditionalSettings.StripedStopByUndestroyable) break;
                if (!square.IsNone())
        ...
        public List<Item> GetRow(Square square)
        {
            ...
            for (var col = square.col; col < levelData.maxCols; col++)
            {
                ...
                if (square1.type == SquareTypes.UndestrBlock &&
                    LevelManager.THIS.AdditionalSettings.StripedStopByUndestroyable) break;
        ...
        public List<Item> GetItemsAround8(Square square)
        {
            ...
            for (var r = row - 1; r <= row + 1; r++)
            {
                for (var c = col - 1; c <= col + 1; c++)
                {
                    ...
                    Physics2D.LinecastNonAlloc(square.transform.position, pos, result,
                        1 << LayerMask.NameToLayer("Item"));
                    var hit = result.Where(i => i.transform).OrderBy(i => (i.transform.position - pos).magnitude)
                        .FirstOrDefault();
