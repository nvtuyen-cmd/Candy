...
namespace Eran
{
    ...
    internal class SysWindowGui : IEcsRunSystem, IEcsInitSystem
    {
        ...
        void ShowTabSettings(Config conf)
        {
            using (new EditorGUILayout.VerticalScope())
            {
                ...
                using (new EditorGUILayout.VerticalScope())
                {
                    ...
                    conf.RebuildCacheOnDemand = GUILayout.Toggle(conf.RebuildCacheOnDemand,
                        "Rebuild cache on demand (when scripts are updated often)");
                    conf.UpdateUnusedAssetsOnDemand = GUILayout.Toggle(conf.UpdateUnusedAssetsOnDemand,
                        "Update unused assets on demand");
                    EditorGUILayout.Space();
        ...
        BacklinkStore.UnusedQty ShowObjectName(BacklinkStore store, WindowData windowData,
            TargetTypeEnum targetTypeEnum, SearchArg arg, string[] selectedGuids)
        {
            float TextWidth()
            {
            ...
            _contentBuf.text = isMultiSelect ? "Multiple" : "No Multiple";
